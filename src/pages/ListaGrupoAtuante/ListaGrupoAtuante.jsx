...
const ListaGrupoAtuante = () => {
	...
	const handleCadastrar = async () => {
		...
		const resCadastrar = await dispatch(
			postGrupoAtuanteAction(token, cadastro.nome)
		);
		if (resCadastrar) {
	...
	const Editar = (row) => {
		...
		const handleEditarCadastro = async () => {
			setLoading(true);
			const resCadastrar = await dispatch(
				putGrupoAtuanteAction(token, cadastroEditar.nome, cadastroEditar.id)
			);
			if (resCadastrar) {
		...
		const handleExcluirCadastro = async (row) => {
			...
			const resDelCandidato = await dispatch(
				delGrupoAtuanteAction(token, row.row.id)
			);
			if (resDelCandidato) {
				setLoading(false);
	...
	return (
		<Box className={classes.root}>
			...
			<Box className={classes.tableContainer}>
				{grupoAtuante.data && grupoAtuante.per_page ? (
					<CustomTable
						columns={columns}
						data={grupoAtuante.data}
						Editar={Editar}
					/>
				) : (
					<Box>
...
export default ListaGrupoAtuante;
