...
const ListaEspecialidade = () => {
	...
	const handleCadastrar = async () => {
		...
		const resCadastrar = await dispatch(
			postEspecialidadeAction(token, cadastro.nome)
		);
		if (resCadastrar) {
	...
	const Editar = (row) => {
		...
		const handleExcluirCadastro = async (row) => {
			...
			const resDelCandidato = await dispatch(
				delEspecialidadeAction(token, row.row.id)
			);
			if (resDelCandidato) {
				setLoading(false);
	...
	return (
		<Box className={classes.root}>
			...
			<Box className={classes.tableContainer}>
				{especialidade.data && especialidade.per_page ? (
					<CustomTable
						columns={columns}
						data={especialidade.data}
						Editar={Editar}
					/>
				) : (
					<Box>
